...
describe('AttemptController', () => {
  ...
  describe('findById()', () => {
    ...
    // if id is present return attempt
    it(`should return an attempt if id is present`, async () => {
      const response = await controller.findById(attemptId.toString());
      const attempt = response?.data;
      expect(attempt.id).toBe(attemptId);
    });
    // if id is not present throw not found exception
    it(`should throw not found exception if id is not present`, async () => {
      const response = controller.findById('hehehehe');
